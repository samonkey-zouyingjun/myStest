...
import com.evideo.sambaprovider.nativefacade.SmbFileHttpWrapper;
import com.evideo.sambaprovider.util.SambaUtil;
...
public class NanoStreamer extends NanoHTTPD implements IStreamer {
    ...
    private Response respond(Map<String, String> headers, String uri) {
        ...
        try {
            if (SambaUtil.isSmbUrl(smbUri) && !TextUtils.isEmpty(mimeTypeForFile)) {
                SmbFileHttpWrapper smbFileHttpWrapper = new SmbFileHttpWrapper(0,0);
                //todo setDate

                InputStream copyStream = new BufferedInputStream(new SmbFileInputStream(null));
                //todo setInputStream

                response = serveSmbFile(smbUri, headers, copyStream, smbFileHttpWrapper, mimeTypeForFile);
            } else {
    ...
    /**
     * 响应请求
     * @param smbFileUrl "smb=;zyj:123456@192.168.199.127/share/test6.rmvb"
     * @param header     请求头
     * @param is         封装InputStream
     * @param smbFile    封装File
     * @param mime       "video/rmvb"
     * @return
     */
    private Response serveSmbFile(String smbFileUrl, Map<String, String> header, InputStream is,
                                  SmbFileHttpWrapper smbFile, String mime) {
        Response res;
        try {
            // Calculate etag
            String eTag = Integer.toHexString((new StringBuilder(smbFile.getName()).
                    append(smbFile.getLastModified()).append(smbFile.getLength())).hashCode());
            ...
            long fileLen = smbFile.getLength();
            if (range != null && startFrom >= 0) {
                if (startFrom >= fileLen) {
                    ...
                    is.skip(startFrom);
                    res = createNonBufferedResponse(Response.Status.PARTIAL_CONTENT, mime,
                            is, fileLen);
